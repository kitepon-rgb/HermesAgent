"""SQLite-backed OAuth 2.1 provider for X-HERMES-MCP.

Single-user, single-process. Clients (DCR), authorization codes, access tokens,
refresh tokens persist to SQLite so container restarts don't invalidate
already-issued tokens or registered DCR clients.

Pending consent sessions (10-minute TTL) stay in memory.

Consent flow (called by the SDK's auto-generated `/authorize` handler):

  1. SDK calls `authorize()` with `params` (PKCE challenge, redirect_uri,
     state, scopes). We mint a one-shot `session_id`, stash the
     (client, params) tuple, and return the URL of our `/consent` page.
  2. The user's browser hits `/consent?session_id=...`. The page asks for
     the master password (env: `MCP_ADMIN_PASSWORD`).
  3. POST returns to `approve_consent()`, which mints an authorization
     code and returns the redirect URL (with `code` + `state`) back to
     the client's `redirect_uri`.
  4. The MCP client exchanges code → token via `/token`, which calls
     `load_authorization_code()` and `exchange_authorization_code()`.
     PKCE verification happens inside the SDK.

Pattern adapted from ip-mcp (same author).
"""
...
class SqliteOAuthProvider(OAuthProvider):
    ...
    FastMCP の `OAuthProvider` を継承するので、SDK 経由で `/authorize`
    `/token` `/register` `/revoke` `/.well-known/oauth-authorization-server`
    `/.well-known/oauth-protected-resource` のルートが自動で生える。
    consent ページ (`/consent` GET+POST) は別途 `pages.py` で組み込む。
    """
    ...
    def __init__(
        ...
        *,
        ...
        # consent ページの絶対 URL。SDK の /authorize ハンドラはここに
        # ブラウザをリダイレクトする。
        self._consent_url = f"{str(base_url).rstrip('/')}/consent"
