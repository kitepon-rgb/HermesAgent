...
| `generate_image` | xAI `grok-imagine-image` を OAuth で直叩き | 約 5〜20 秒 | `{url, mime_type, model, cost_in_usd_ticks, …}` |

V4 バックエンドは Grok-4.3 の合成段をバイパスして Responses API を直接叩く形 — 出力決定性、ISO 8601 タイムスタンプ、metrics は exact integer、レスポンスに `source: "x_search_raw_v4"` の provenance タグが付く。`generate_image` は同じ OAuth bearer で `api.x.ai/v1/images/generations` を直接 POST する経路で、課金経路は SuperGrok / Premium Plus quota のみ。V3 ラップ → V4 raw → 画像生成追加までの経緯は [`DOCS/plan.md`](DOCS/plan.md) に記録。
