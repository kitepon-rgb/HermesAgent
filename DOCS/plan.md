...
## Phase 10 — generate_image の表示パイプライン整備 (2026-05-18 完了)

### 経緯と切り分け

Phase 9 で generate_image が成功するも、Claude.ai web / ChatGPT web の **どちらも画像が UI に出ない**。最初は転送サイズ起因と仮定して 768→512、quality 75→60 と圧縮を強めたが効果なし。MCP / claude.ai / ChatGPT の公式 issue を調べた結果、**プラットフォーム固有の既知制約**と確定:

- claude.ai web: MCP `image` content block は UI に描画されない (anthropic/claude-ai-mcp #238)。LLM 自体はバイトを受け取り推論できる
- Claude Desktop: ツール結果パネル内に折りたたみで描画
- Claude Code (CLI / VSCode): インライン描画 OK
- ChatGPT (Web / Desktop): `type:"image"` は **空 `{}`** として返す / 生 base64 を text として LLM に流すだけ。MCP 由来の `![](url)` markdown も描画しない (openai community #1375446, #1380737)
- OpenAI 公式 workaround: `content` text / `structuredContent` で **テキスト経路で補完** しろ

ソース: [MCP Has an Image Problem](https://patent.dev/model-context-protocol-has-an-image-problem/)、anthropic/claude-ai-mcp #238、openai community #1375446 / #1380737。

サーバ側で塞ぐのは不可能なので、運用フローを「URL を見せる」中心に切り替えた。
...
- [x] Pillow 追加、`_compress_for_inline()` で 512px / q60 に再圧縮 → ImageContent base64 は ~5-50KB に収束

### 戻り値の最終形 (3 ブロック)

```text
[0] ImageContent (~5-50KB JPEG)
    UI には描画されない (web client) が、LLM が「生成画像と prompt が
...
[1] TextContent (user-facing 1 行)
    ...
    そのまま転載しやすい短い形にしておく
[3] TextContent (JSON metadata)
    permanent_url / url / mime_type / model / prompt / aspect_ratio /
    resolution / cost_in_usd_ticks / source — programmatic chaining 用
```

エラー時 (`call_image_generate` 失敗 / xAI download 失敗 / disk 書込失敗) も同じ user-facing text 経路で「失敗した + 理由」を出す。LLM が「完了しました!」と無内容に返すのを防ぐ。

### 試行錯誤メモ (戻り値スキーマ)

| 試行 | 結果 |
|---|---|
| `[dict]` のみ | claude.ai web で「outputSchema defined but no structured output」エラー → `@mcp.tool(output_schema=None)` で skip |
| `[ImageContent, dict]` | claude.ai web 1 回目だけ画像表示、2 回目以降「Error during tool execution」(payload size 起因疑い) |
| `[ImageContent(small), TextContent(user), TextContent(json)]` | 安定。ただし web 側は画像 UI 非表示、URL クリックで見る運用 |
| `[TextContent(user), TextContent(json)]` (image 削除) | LLM が画像を見れなくなり「何が生成されたか」grade できない → revert |

---

## 将来の拡張: 他領域の Hermes MCP (Phase 11 以降の候補)
